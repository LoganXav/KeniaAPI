...
import DeleteStaffService from "../services/DeletStaff.service"
  ...
  export default class StaffUpdateController extends BaseController {
    ...
    deleteOneStaff: EntryPointHandler = async (
      req: IRequest,
      res: IResponse,
      next: INextFunction
    ): Promise<void> => {
      return this.handleResultData(
        res,
        next,
        this.deleteStaffService.execute(res.trace, req.body),
        {
    ...
    deleteStaffs: EntryPointHandler = async (
        req: IRequest,
        res: IResponse,
        next: INextFunction
      ): Promise<void> => {
        return this.handleResultData(
          res,
          next,
          this.deleteStaffService.deleteStaffs(res.trace, req.body),
          {
            [HttpHeaderEnum.CONTENT_TYPE]: HttpContentTypeEnum.APPLICATION_JSON
          }
        )
