...
export default class SubjectGradingStructureCreateProvider {
  public async create(args: SubjectGradingStructureCreateRequestType, dbClient: PrismaTransactionClient = DbClient): Promise<SubjectGradingStructure> {
    try {
      const { subjectId, tenantGradingStructureId, continuousAssessmentBreakdownItems, tenantId, staffId } = args;

      const subjectGradingStructure = await dbClient.subjectGradingStructure.create({
        data: {
