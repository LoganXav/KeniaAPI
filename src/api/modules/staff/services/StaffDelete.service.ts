...
export default class DeleteStaffService extends BaseService<any> {
  ...
  public async execute(trace: ServiceTrace, args: StaffCriteriaType): Promise<IResult> {
    try {
      ...
      if (staff) {
        trace.setSuccessful();
        this.result.setData(SUCCESS, HttpStatusCodeEnum.SUCCESS, `Delete Staff Information!!!`, staff);
        return this.result;
      } else {
  ...
  public async deleteStaffs(trace: ServiceTrace, args: StaffCriteriaType): Promise<IResult> {
    try {
      ...
      if (staffs) {
        trace.setSuccessful();
        this.result.setData(SUCCESS, HttpStatusCodeEnum.SUCCESS, `Delete Staffs Information`, staffs);
        return this.result;
      } else {
