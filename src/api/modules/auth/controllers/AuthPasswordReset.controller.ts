...
export default class AuthPasswordResetController extends BaseController {
  ...
  constructor(authPasswordResetRequestService: AuthPasswordResetRequestService, authPasswordResetService: AuthPasswordResetService) {
    super();
  ...
  resetRequest: EntryPointHandler = async (req: IRequest, res: IResponse, next: INextFunction): Promise<void> => {
    return this.handleResultData(res, next, this.authPasswordResetRequestService.execute(res.trace, req.body), {
      [HttpHeaderEnum.CONTENT_TYPE]: HttpContentTypeEnum.APPLICATION_JSON,
    });
  };
  reset: EntryPointHandler = async (req: IRequest, res: IResponse, next: INextFunction): Promise<void> => {
